...
pub mod config {
    ...
    impl Config {
        ...
        pub fn is_permission_profile_allowed(
            ...
            self.custom_permission_profiles
                .iter()
    ...
    pub mod edit {
        ...
        /// Browser facade for the 0.145 config-edit builder.
        ///
        /// Config persistence still requires a VFS host operation, so applying
        /// an edit reports that limitation instead of pretending it was saved.
        pub struct ConfigEditsBuilder {
            resume_cwd: Option<ResumeCwdMode>,
        }
        ...
        impl ConfigEditsBuilder {
            ...
            pub fn set_resume_cwd(mut self, mode: ResumeCwdMode) -> Self {
                self.resume_cwd = Some(mode);
                self
            }

            pub async fn apply(self) -> std::io::Result<()> {
                let _ = self.resume_cwd;
                Err(std::io::Error::new(
                    std::io::ErrorKind::Unsupported,
                    "writing config.toml in the browser requires a VFS host operation",
                ))
            }
        }
    }
