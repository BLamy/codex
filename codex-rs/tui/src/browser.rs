...
#[cfg(all(target_arch = "wasm32", feature = "real-tui-wasm"))]
fn new_agent_message(markdown_source: String, cwd: &std::path::Path) -> AgentMarkdownCell {
...
fn browser_tui_version_for_env(_env: &[(String, String)]) -> Option<String> {
    browser_tui_version()
}
...
impl BrowserInteractiveTuiSession {
    ...
    fn push_history_cell(
        ...
        self.pending_scrollback_lines
            .extend(cell.display_lines(width));
...
impl RealBrowserInteractiveTuiSession {
    fn new(options: &BrowserTuiSessionOptions) -> Result<Self, String> {
        ...
        let common = ChatWidgetInit {
            ...
            has_codex_backend_auth: has_auth,
    ...
    fn start(
        ...
        if !prompt.trim().is_empty() {
            self.chat.replace_browser_composer_text(prompt);
    ...
    fn set_input(
        ...
        self.chat.replace_browser_composer_text(input);
        ...
        while let Ok(event) = self.app_event_rx.try_recv() {
            match event {
                ...
                AppEvent::ClearUiAndSubmitUserMessage { text } => {
                    self.transcript_cells.clear();
                    self.chat.replace_browser_composer_text(text);
                    self.chat
                        .handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
                }
                _ => {}
...
fn prompt_from_user_inputs(items: &[UserInput]) -> String {
    items
        ...
        .filter_map(|item| match item {
            ...
            UserInput::Audio { url } => Some(format!("[audio: {url}]")),
            UserInput::LocalAudio { path } => Some(format!("[local audio: {}]", path.display())),
...
mod tests {
    ...
    #[test]
    fn browser_session_header_uses_compiled_version() {
        let mut session = BrowserInteractiveTuiSession::default();
        let result = session
            .start(
                None,
                &BrowserTuiSessionOptions {
                    env: vec![("CODEX_CLI_VERSION".to_string(), "spoofed".to_string())],
                    ..BrowserTuiSessionOptions::default()
                },
            )
            .expect("start frame");
