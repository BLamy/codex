...
impl NetworkProxyState {
    ...
    pub async fn current_patterns(&self) -> Result<(Vec<String>, Vec<String>)> {
        ...
        Ok((
            cfg.allowed_domains().unwrap_or_default(),
            cfg.denied_domains().unwrap_or_default(),
        ))
    }

    pub async fn enabled(&self) -> Result<bool> {
        Ok(self.current_cfg().await?.enabled)
    }
    ...
    pub async fn method_allowed(&self, method: &str) -> Result<bool> {
        Ok(self.current_cfg().await?.mode.allows_method(method))
    }

    pub async fn allow_upstream_proxy(&self) -> Result<bool> {
        Ok(self.current_cfg().await?.allow_upstream_proxy)
    }

    pub async fn allow_local_binding(&self) -> Result<bool> {
        Ok(self.current_cfg().await?.allow_local_binding)
    }

    pub async fn network_mode(&self) -> Result<NetworkMode> {
        Ok(self.current_cfg().await?.mode)
    }

    pub async fn set_network_mode(&self, mode: NetworkMode) -> Result<()> {
        self.state.write().unwrap().config.mode = mode;
        Ok(())
    }
    ...
    pub async fn add_allowed_domain(&self, host: &str) -> Result<()> {
        self.state.write().unwrap().config.upsert_domain_permission(
            host.to_string(),
...
/// Portable managed-network facts needed by a host sandbox.
...
impl NetworkProxy {
    ...
    pub fn for_execution(
        ...
        if self.execution_scope.is_some() {
            return Err(anyhow!("cannot scope an execution-scoped network proxy"));
        }
        let mut proxy = self.clone();
        proxy.execution_scope = Some(Arc::new(ExecutionScope {
            environment_id: environment_id.to_string(),
            execution_id: execution_id.to_string(),
            _attribution_token: attribution_token,
        }));
        Ok(proxy)
    }
    ...
    pub fn prepare_for_optional_environment(
        ...
        self.apply_to_env_for_optional_environment(&mut env, environment_id)?;
