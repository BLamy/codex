...
///
/// Browser Codex does not open websocket or stdio exec-server transports from
/// Rust. Instead, [`crate::EnvironmentManager`] installs host-backed process,
/// filesystem, and HTTP implementations directly.
#[derive(Clone, Debug)]
...
///
/// The browser host runtime currently buffers `network/fetch` responses at the
/// host bridge boundary and exposes them through the same pull API as native
/// streamed responses. This keeps upstream callers on `HttpClient` while the
/// host bridge grows true body-delta streaming.
