...
fn apply_turn_context(metadata: &mut ThreadMetadata, turn_ctx: &TurnContextItem) {
    if metadata.cwd.as_os_str().is_empty() {
        metadata.cwd = turn_ctx.cwd.as_path().to_path_buf();
    }
...
impl StateRuntime {
    ...
    pub async fn find_rollout_path_by_id(
        ...
        Ok(self.get_thread(id).await?.and_then(|thread| {
            let is_archived = thread.archived_at.is_some();
            archived_only
                .is_none_or(|archived_only| archived_only == is_archived)
                .then_some(thread.rollout_path)
        }))
    }
    ...
    pub async fn find_thread_by_exact_title(
        ...
        Ok(self
            ...
            .map_err(|_| lock_err())?
            .values()
            .filter(|thread| thread.archived_at.is_some() == archived_only)
            .filter(|thread| {
    ...
    fn list_threads_matching(
        &self,
        page_size: usize,
        filters: ThreadFilterOptions<'_>,
        relation_filter: Option<ThreadRelationFilter>,
    ) -> anyhow::Result<ThreadsPage> {
        let related_parent_ids = match relation_filter {
            Some(ThreadRelationFilter::DirectChildrenOf(parent)) => self
                .list_thread_spawn_children_matching(parent, None)?
                .into_iter()
                .map(|child| (child, parent))
                .collect::<HashMap<_, _>>(),
            Some(ThreadRelationFilter::DescendantsOf(ancestor)) => {
                let descendants = self.list_thread_spawn_descendants_matching(ancestor, None)?;
                let edges = self.spawn_edges.lock().map_err(|_| lock_err())?;
                descendants
                    .into_iter()
                    .filter_map(|child| {
                        let parent = edges
                            .get(&child.to_string())
                            .and_then(|(parent, _)| ThreadId::try_from(parent.clone()).ok())?;
                        Some((child, parent))
                    })
                    .collect()
            }
        ...
        let mut items: Vec<_> = self
            ...
            .values()
            .filter(|thread| {
                relation_filter.is_none() || related_parent_ids.contains_key(&thread.id)
            })
        ...
        items.sort_by_key(|thread| match filters.sort_key {
            ...
            SortKey::RecencyAt => thread.recency_at,
        });
        ...
        if let Some(anchor) = filters.anchor {
            items.retain(|thread| match (filters.sort_key, filters.sort_direction) {
                ...
                (SortKey::RecencyAt, SortDirection::Asc) => {
                    thread.recency_at > anchor.ts
                        || (thread.recency_at == anchor.ts
                            && anchor
                                .id
                                .is_some_and(|id| thread.id.to_string() > id.to_string()))
                }
                (SortKey::RecencyAt, SortDirection::Desc) => {
                    thread.recency_at < anchor.ts
                        || (thread.recency_at == anchor.ts
                            && anchor
                                .id
        ...
        let next_anchor = if items.len() > page_size {
            ...
            items
                .last()
                .map(|item| anchor_from_item(item, filters.sort_key, relation_filter.is_some()))
        } else {
            None
        };
        let parent_thread_ids = items
            .iter()
            .filter_map(|item| {
                related_parent_ids
                    .get(&item.id)
                    .copied()
                    .map(|parent| (item.id, parent))
            })
            .collect();
        Ok(ThreadsPage {
            items,
    ...
    pub async fn update_thread_name(
        ...
        thread.name = name.map(ToString::to_string);
        Ok(true)
    }
    ...
    pub async fn delete_threads_strict(&self, thread_ids: &[ThreadId]) -> anyhow::Result<u64> {
        ...
        for thread_id in thread_ids {
            ...
            self.spawn_edges
                .lock()
                .map_err(|_| lock_err())?
                .retain(|child, (parent, _)| {
                    child != &thread_id.to_string() && parent != &thread_id.to_string()
                });
