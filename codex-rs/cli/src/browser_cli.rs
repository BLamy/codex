...
impl BrowserCodexCliSession {
    ...
    pub fn run(&mut self, args: Vec<String>, options: BrowserRunOptions) -> BrowserRunResult {
        if matches!(args.as_slice(), [arg] if arg == "--version" || arg == "-V") {
            return BrowserRunResult::stdout(format!("codex {}\n", crate::CODEX_CLI_VERSION), 0);
        }
        ...
        match args.first().map(String::as_str) {
            ...
            Some("browser-tui-notification") => {
                ...
                match parse_browser_tui_notification_args(args) {
                    Ok(notification) => {
                        browser_tui_result(self.tui.apply_server_notification_json(
                            notification,
...
fn parse_exec_plan(
    ...
    Ok(BrowserExecPlan {
        ...
        apply_patch_grammar: Some(
            include_str!("../../core/src/tools/handlers/apply_patch.lark").to_string(),
        ),
    })
}
...
mod tests {
    ...
    #[test]
    fn version_uses_compiled_upstream_version() {
        let mut session = BrowserCodexCliSession::new();
