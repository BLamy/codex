...
impl ExecutorSkillProvider {
    ...
    async fn list_inner(&self, query: SkillListQuery) -> SkillProviderResult<SkillCatalog> {
...
impl SkillProvider for ExecutorSkillProvider {
    fn list(&self, query: SkillListQuery) -> SkillProviderFuture<'_, SkillCatalog> {
        ...
        {
            Box::pin(self.list_inner(query))
        }
        #[cfg(target_arch = "wasm32")]
        {
            let provider = self.clone();
            bridge_local_provider_future(async move { provider.list_inner(query).await })
        }
    }
